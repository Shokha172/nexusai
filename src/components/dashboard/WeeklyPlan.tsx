  ...
  return (
    <div className="animate-fade-in max-w-4xl mx-auto space-y-6">
      <div className="flex justify-between items-end mb-6">
        <div>
          <h2 className="text-2xl font-bold text-foreground mb-2">Weekly Action Plan</h2>
          <p className="text-muted-foreground text-sm">Shu hafta uchun NEXUS AI tomonidan tuzilgan vazifalar</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Calendar Card */}
        <Card className="bg-background border-border p-0 md:col-span-1 shadow-xl">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-6">
              <span className="text-foreground font-bold">Joriy Hafta</span>
              <CalendarIcon className="w-5 h-5 text-emerald-500" />
            </div>
            <div className="flex justify-between text-center text-xs text-muted-foreground">
               <div className="flex flex-col gap-2"><span className="text-zinc-600">Du</span><span className="w-8 h-8 flex items-center justify-center rounded-full bg-card text-foreground border border-border">12</span></div>
               ...
               <div className="flex flex-col gap-2"><span className="text-zinc-600">Ch</span><span className="w-8 h-8 flex items-center justify-center rounded-full text-muted-foreground hover:bg-card transition-colors">14</span></div>
               <div className="flex flex-col gap-2"><span className="text-zinc-600">Pa</span><span className="w-8 h-8 flex items-center justify-center rounded-full text-muted-foreground hover:bg-card transition-colors">15</span></div>
               <div className="flex flex-col gap-2"><span className="text-zinc-600">Ju</span><span className="w-8 h-8 flex items-center justify-center rounded-full text-muted-foreground hover:bg-card transition-colors">16</span></div>
            </div>
            
            <div className="mt-8 pt-6 border-t border-border text-center">
              <p className="text-sm font-mono text-muted-foreground mb-2">Progress</p>
        ...
        {/* Task List */}
        <Card className="bg-background border-border p-0 md:col-span-2 relative min-h-[300px] shadow-xl">
           <CardContent className="p-6">
             ...
             {isLoading ? (
               <div className="absolute inset-0 flex flex-col items-center justify-center text-emerald-500 animate-pulse bg-background/50 backdrop-blur-sm z-10">
                 ...
                 {tasks.map((task, i) => (
                   ...
                   >
                     ...
                     <div className="flex-1">
                       <p className={`text-sm font-medium transition-colors ${task.status === 'done' ? 'text-muted-foreground line-through' : 'text-zinc-200'}`}>
                         {task.text}
                       </p>
