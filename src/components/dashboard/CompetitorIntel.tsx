  ...
  return (
    <div className="animate-fade-in max-w-4xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 gap-4">
        <div>
          <h2 className="text-2xl font-bold text-foreground mb-2">Competitor Intelligence</h2>
          <p className="text-muted-foreground text-sm">Yaqin atrofdagi 3 ta asosiy raqobatchi tahlili</p>
        </div>
        ...
        <form onSubmit={handleSearch} className="relative w-full md:w-auto flex items-center">
           <Input 
             ...
             className="w-full md:w-64 pr-10 bg-card/50 border-border rounded-full focus-visible:ring-emerald-500"
           />
           <Button type="submit" variant="ghost" size="icon" className="absolute right-0 hover:bg-transparent text-muted-foreground hover:text-emerald-500 rounded-full h-full">
      ...
      <div className="grid gap-4 relative min-h-[300px]">
        {isLoading && (
          <div className="absolute inset-0 bg-background/80 backdrop-blur-sm z-10 flex flex-col items-center justify-center text-emerald-500 rounded-2xl">
        ...
        {!isLoading && competitors.length === 0 && (
          <div className="text-center text-muted-foreground py-10">Raqobatchilar topilmadi.</div>
        )}

        {competitors.map((comp, i) => (
          <Card key={i} className="bg-background border-border hover:border-border transition-colors p-0 overflow-hidden">
            <CardContent className="p-6 flex flex-col md:flex-row gap-6 justify-between items-start md:items-center">
              <div className="flex-1">
                <h3 className="text-lg font-bold text-foreground mb-2 flex items-center gap-2">
                  {comp.name} 
                  <span className="text-[10px] bg-card px-2 py-0.5 rounded font-mono text-muted-foreground border border-border">{comp.distance}</span>
                </h3>
              ...
              <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto shrink-0">
                 <div className="bg-card/50 border border-border rounded-lg p-3 md:w-40">
