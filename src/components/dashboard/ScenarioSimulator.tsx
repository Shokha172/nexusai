    ...
    <div className="animate-fade-in max-w-4xl mx-auto space-y-6">
      <div className="flex justify-between items-end mb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm">"What-if" tahlillari va turli biznes ssenariylarini tekshirish</p>
        </div>
      </div>
      ...
      <div className="bg-[#0a0b10] border border-slate-800 rounded-2xl p-6 shadow-xl">
        <div className="flex items-center gap-3 mb-6">
           ...
           <div>
             <h3 className="font-bold text-white">Ssenariy kiritish</h3>
             <p className="text-xs text-slate-500">O'zgartirish kiritib natijani prognoz qiling.</p>
           </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <input 
            ...
            className="flex-1 bg-[#111] border border-slate-700 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-purple-500 transition-colors"
          />
          <button 
            onClick={handleSimulate}
            className="bg-purple-600 hover:bg-purple-500 text-white font-bold py-3 px-6 rounded-xl transition-all flex items-center justify-center gap-2 shadow-lg shadow-purple-900/20"
          >
            {loading ? (
               <span className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></span>
            ) : (
               <><Play className="w-4 h-4 fill-white" /> Simulyatsiya</>
            )}
          </button>
        </div>
