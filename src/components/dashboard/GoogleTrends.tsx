  ...
  return (
    <div className="animate-fade-in max-w-4xl mx-auto space-y-6">
      <div className="flex justify-between items-end mb-6">
        <div>
          <h2 className="text-2xl font-bold text-white mb-2">Trend Analysis</h2>
          <p className="text-slate-400 text-sm">"{dna.industry || 'Biznes'}" bo'yicha qidiruv trendlari ({dna.district || 'Uzbekistan'})</p>
        </div>
      </div>

      <div className="bg-[#0a0b10] border border-slate-800 rounded-2xl p-8 relative min-h-[300px]">
        {isLoading && (
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white font-bold">Google Trends (AI Simulated)</p>
              <p className="text-xs text-slate-400">So'nggi 12 oy</p>
            </div>
          </div>
          <div className="text-right">
            ...
            <span className="text-xs text-slate-500">Talab {trends?.growthPercent > 0 ? '+' : ''}{trends?.growthPercent || 0}% o'zgargan</span>
          </div>
        </div>

        {/* Dynamic Chart */}
        <div className="h-48 w-full border-b border-l border-slate-800 relative flex items-end justify-between px-2 pb-0 pt-8">
           {/* Grid lines */}
           <div className="absolute top-0 left-0 w-full h-px bg-slate-800/50"></div>
           <div className="absolute top-1/2 left-0 w-full h-px bg-slate-800/50"></div>
           ...
           {trends?.data?.map((h: number, i: number) => (
             <div key={i} className="w-8 md:w-12 bg-gradient-to-t from-blue-600/20 to-blue-500 rounded-t-sm relative group cursor-pointer transition-all hover:opacity-80" style={{ height: `${h}%` }}>
               <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-white text-black text-[10px] font-bold px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity z-20">
                 {h}
               </div>
             </div>
           ))}
        </div>
        <div className="flex justify-between mt-2 text-[10px] font-mono text-slate-500 px-2">
