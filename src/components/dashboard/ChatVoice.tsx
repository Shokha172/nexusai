...
export default function ChatVoice({ dna }: ChatVoiceProps) {
  ...
  return (
    <div className="animate-fade-in max-w-4xl mx-auto h-[80vh] flex flex-col border border-border bg-background rounded-2xl overflow-hidden shadow-2xl">
      {/* Header */}
      <div className="p-4 border-b border-border bg-card flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-bold text-foreground text-sm">NEXUS AI Advisor</h2>
            <p className="text-[10px] font-mono text-emerald-400 flex items-center gap-1">
              <Sparkles className="w-3 h-3" /> Powered by OpenAI
            </p>
          </div>
        </div>
      </div>
            ...
            <div className={`max-w-[80%] flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
              ...
              <div className={`p-4 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap ${
                msg.role === 'user' 
                  ...
                  : 'bg-muted/50 border border-border text-muted-foreground rounded-tl-sm'
              }`}>
                {msg.content}
                {msg.role === 'assistant' && i !== 0 && !isTyping && (
                  <div className="mt-3 pt-3 border-t border-border/50 flex gap-2">
                    <Button 
                      ...
                      className="text-[10px] h-6 px-2 font-mono flex items-center gap-1 text-muted-foreground hover:text-emerald-400 transition-colors"
                    >
        ...
        <div className="flex items-center gap-2 relative">
          <Button 
            ...
            className={`w-12 h-12 rounded-full flex items-center justify-center transition-all shrink-0 ${
              isRecording ? 'animate-pulse' : 'bg-muted text-muted-foreground hover:text-foreground hover:bg-zinc-700'
            }`}
          >
          ...
          <div className="flex-1 relative">
            {isRecording && !input ? (
              <div className="h-12 w-full bg-card border border-border rounded-full px-6 flex items-center gap-3">
                <span className="w-2 h-2 rounded-full bg-rose-500 animate-pulse"></span>
                <span className="text-sm font-mono text-muted-foreground">Tinglanmoqda... Gapiring</span>
              </div>
            ) : (
              <Input
                type="text"
                ...
                className="w-full h-12 bg-card/50 border-border text-foreground text-sm rounded-full px-6 focus-visible:ring-emerald-500 transition-colors"
              />
            )}
          </div>
          ...
          <Button 
            ...
            className={`w-12 h-12 rounded-full flex items-center justify-center transition-all shrink-0 ${
              input.trim() && !isTyping ? 'bg-emerald-500 text-zinc-950 hover:bg-emerald-400' : 'bg-muted text-muted-foreground cursor-not-allowed'
            }`}
          >
